...
import com.ly.fn.motanx.api.transport.netty.NettyEndpointFactory;
import com.ly.fn.motanx.api.transport.support.HeartbeatClientEndpointManager;
...
public class NettyEndpointFactoryTest extends TestCase {
    ...
    private void testNotShareChannel(boolean isServer) {
        ...
        URL url = new URL("motanx", "localhost", 18080, "com.weibo.api.motan.procotol.example.IHello");

        Endpoint endpoint = createEndpoint(url, handler, isServer, factory);

        Assert.assertEquals(endpoint.getUrl().getUri(), url.getUri());

        url = new URL("motanx", "localhost", 18081, "com.weibo.api.motan.procotol.example.IHello");
        endpoint = createEndpoint(url, handler, isServer, factory);
        Assert.assertEquals(endpoint.getUrl().getUri(), url.getUri());

        Assert.assertTrue(endpoint != createEndpoint(new URL("motanx", "localhost", 18081, "com.weibo.api.motan.procotol.example.IHello"),
