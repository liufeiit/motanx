...
import com.ly.fn.motanx.api.rpc.Application;
...
public class StatsUtil {
    ...
    static {
        ...
        }, MotanxConstants.STATISTIC_PEROID, MotanxConstants.STATISTIC_PEROID, TimeUnit.SECONDS);
    }
    ...
    public static void accessStatistic(String name, Application application, long currentTimeMillis, long costTimeMillis, long bizProcessTime, AccessStatus accessStatus) {
    ...
    public static ConcurrentMap<String, AccessStatisticResult> getTotalAccessStatistic() {
        return getTotalAccessStatistic(MotanxConstants.STATISTIC_PEROID);
    }

    public static ConcurrentMap<String, AccessStatisticResult> getTotalAccessStatistic(int peroid) {
        if (peroid > MotanxConstants.STATISTIC_PEROID) {
            throw new RuntimeException("peroid need <= " + MotanxConstants.STATISTIC_PEROID);
        }
        ...
        for (Map.Entry<String, AccessStatisticItem> entry : accessStatistics.entrySet()) {
            ...
            AccessStatisticResult result = item.getStatisticResult(currentTimeMillis, MotanxConstants.STATISTIC_PEROID);
    ...
    public static void logAccessStatistic(boolean clear) {
        ...
        for (Map.Entry<String, AccessStatisticItem> entry : accessStatistics.entrySet()) {
            ...
            if (clear) {
                item.clearStatistic(currentTimeMillis, MotanxConstants.STATISTIC_PEROID);
            }
            ...
            Snapshot snapshot = InternalMetricsFactory.getRegistryInstance(entry.getKey()).histogram(MetricRegistry.name(AccessStatisticItem.class, "costTimeMillis")).getSnapshot();
            ...
            if (result.totalCount == 0) {
                ...
                LoggerUtil.accessStatsLog(
                        ...
                        application, module, keys[0], result.totalCount, result.slowCount, mbFormat.format(snapshot.get75thPercentile()), mbFormat.format(snapshot.get95thPercentile()),
                        mbFormat.format(snapshot.get98thPercentile()), mbFormat.format(snapshot.get99thPercentile()), mbFormat.format(snapshot.get999thPercentile()), result.bizExceptionCount,
                        result.otherExceptionCount, mbFormat.format(result.costTime / result.totalCount), mbFormat.format(result.bizTime / result.totalCount),
                        (result.totalCount / MotanxConstants.STATISTIC_PEROID), result.maxCount, result.minCount);
            }
            ...
            for (Map.Entry<String, AccessStatisticResult> entry : totalResults.entrySet()) {
                ...
                if (totalResult.totalCount > 0) {
                    LoggerUtil.accessStatsLog(
                            ...
                            application, module, totalResult.totalCount, totalResult.slowCount, mbFormat.format(snapshot.get75thPercentile()), mbFormat.format(snapshot.get95thPercentile()),
                            mbFormat.format(snapshot.get98thPercentile()), mbFormat.format(snapshot.get99thPercentile()), mbFormat.format(snapshot.get999thPercentile()), totalResult.bizExceptionCount,
                            totalResult.otherExceptionCount, mbFormat.format(totalResult.costTime / totalResult.totalCount), mbFormat.format(totalResult.bizTime / totalResult.totalCount),
                            (totalResult.totalCount / MotanxConstants.STATISTIC_PEROID));
                } else {
...
class AccessStatisticItem {
    ...
    public AccessStatisticItem(String name, long currentTimeMillis) {
        this(name, currentTimeMillis, MotanxConstants.STATISTIC_PEROID * 2);
    }
    ...
    void statistic(long currentTimeMillis, long costTimeMillis, long bizProcessTime, AccessStatus accessStatus) {
        ...
        if (costTimeMillis >= MotanxConstants.SLOW_COST) {
        ...
        InternalMetricsFactory.getRegistryInstance(appName).histogram(MetricRegistry.name(AccessStatisticItem.class, "costTimeMillis")).update(costTimeMillis);
    }
