...
public class ProviderProtectedMessageRouter extends ProviderMessageRouter {
    ...
    protected Response call(Request request, Provider<?> provider) {
        ...
        String requestKey = MotanxFrameworkUtil.getFullMethodString(request);
    ...
    private Response reject(String method, int requestCounter, int totalCounter, int maxThread) {
        ...
        MotanxServiceException exception =
                new MotanxServiceException("ThreadProtectedRequestRouter reject request: request_counter=" + requestCounter + " total_counter=" + totalCounter + " max_thread=" + maxThread,
                        MotanxErrorMsgConstant.SERVICE_REJECT);
