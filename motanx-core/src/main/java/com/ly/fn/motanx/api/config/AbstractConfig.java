...
public abstract class AbstractConfig implements Serializable {
    ...
    protected static void collectMethodConfigParams(Map<String, String> parameters, List<MethodConfig> methods) {
        ...
        for (MethodConfig mc : methods) {
            if (mc != null) {
                mc.appendConfigParams(parameters, MotanxConstants.METHOD_CONFIG_PREFIX + mc.getName() + "(" + mc.getArgumentTypes() + ")");
            }
        }
    }
    ...
    protected void appendConfigParams(Map<String, String> parameters, String prefix) {
        ...
        for (Method method : methods) {
            try {
                ...
                if (isConfigMethod(method)) {
                    ...
                    if (value == null || StringUtils.isBlank(String.valueOf(value))) {
                        if (configDesc != null && configDesc.required()) {
                            throw new MotanxFrameworkException(String.format("%s.%s should not be null or empty", this.getClass().getSimpleName(), key), MotanxErrorMsgConstant.FRAMEWORK_INIT_ERROR);
                        }
                        continue;
                    }
                ...
                throw new MotanxFrameworkException(String.format("Error when append params for config: %s.%s", this.getClass().getSimpleName(), method.getName()), e,
    ...
    private boolean isConfigMethod(Method method) {
        boolean checkMethod = (method.getName().startsWith("get") || method.getName().startsWith("is")) && !"isDefault".equals(method.getName()) && Modifier.isPublic(method.getModifiers())
    ...
    private boolean isPrimitive(Class<?> type) {
        return type.isPrimitive() || type == String.class || type == Character.class || type == Boolean.class || type == Byte.class || type == Short.class || type == Integer.class
    ...
    public String toString() {
        try {
            ...
            for (Method method : methods) {
                try {
                    String name = method.getName();
                    if ((name.startsWith("get") || name.startsWith("is")) && !"getClass".equals(name) && !"get".equals(name) && !"is".equals(name) && Modifier.isPublic(method.getModifiers())
                            && method.getParameterTypes().length == 0 && isPrimitive(method.getReturnType())) {
