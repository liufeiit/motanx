...
import com.ly.fn.motanx.api.switcher.SwitcherListener;
...
public abstract class AbstractRegistry implements Registry, Runnable {
    ...
    public AbstractRegistry(URL url) {
        ...
        MotanxSwitcherUtil.registerSwitcherListener(MotanxConstants.REGISTRY_HEARTBEAT_SWITCHER, new SwitcherListener() {
            @Override
        ...
        // add by 刘飞 退出jvm的时候注销服务
        Runtime.getRuntime().addShutdownHook(new Thread(this));
        SignalHandler sh = new SignalHandler() {
            public void handle(Signal sn) {
                LoggerUtil.info(sn.getName() + " signal is recevied.");
                System.exit(0);
            }
        };
        try {
    ...
    public void run() {
        ...
        for (URL url : registeredServiceUrls) {
            LoggerUtil.info("Unregister URL : {}", url);
            try {
                doUnregister(removeUnnecessaryParmas(url.createCopy()));
            } catch (Exception e) {
                LoggerUtil.error("Unregister URL : {} Error.", url);
            }
        }
    }
    ...
    public void subscribe(URL url, NotifyListener listener) {
        ...
        LoggerUtil.info("[{}] Listener ({}) will subscribe to url ({}) in Registry [{}]", registryClassName, listener, url, registryUrl.getIdentity());
    ...
    public void unsubscribe(URL url, NotifyListener listener) {
        ...
        LoggerUtil.info("[{}] Listener ({}) will unsubscribe from url ({}) in Registry [{}]", registryClassName, listener, url, registryUrl.getIdentity());
