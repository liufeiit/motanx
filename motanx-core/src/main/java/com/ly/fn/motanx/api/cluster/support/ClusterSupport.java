...
public class ClusterSupport<T> implements NotifyListener {
    ...
    public void init() {
        ...
        throw new MotanxFrameworkException(String.format("ClusterSupport No service urls for the refer:%s, registries:%s",
                this.url.getIdentity(), registryUrls), MotanxErrorMsgConstant.SERVICE_UNFOUND);
    }
    ...
    public void destroy() {
        ...
        for (URL ru : registryUrls) {
            try {
                ...
                if (!MotanxConstants.NODE_TYPE_REFERER.equals(url.getParameter(URLParamType.nodeType.getName()))) {
    ...
    private URL toSubscribeUrl(URL url) {
        ...
        subUrl.addParameter(URLParamType.nodeType.getName(), MotanxConstants.NODE_TYPE_SERVICE);
        return subUrl;
    }
    ...
    private List<URL> parseDirectUrls(String directUrlStr) {
        String[] durlArr = MotanxConstants.COMMA_SPLIT_PATTERN.split(directUrlStr);
