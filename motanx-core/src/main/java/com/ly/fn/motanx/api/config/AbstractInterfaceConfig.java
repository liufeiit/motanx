...
import com.ly.fn.motanx.api.util.UrlUtils;
...
public class AbstractInterfaceConfig extends AbstractConfig {
        ...
        if (registries != null && !registries.isEmpty()) {
            for (RegistryConfig config : registries) {
                ...
                if (StringUtils.isBlank(address)) {
                    address = NetUtils.LOCALHOST + ":" + MotanxConstants.DEFAULT_INT_VALUE;
                }
                ...
                if (!map.containsKey(URLParamType.protocol.getName())) {
                    ...
                    map.put(URLParamType.protocol.getName(), MotanxConstants.REGISTRY_PROTOCOL_LOCAL);
                }
            ...
            for (MethodConfig methodBean : methods) {
                ...
                for (java.lang.reflect.Method method : interfaceClass.getMethods()) {
                    if (method.getName().equals(methodName)) {
                        ...
                        if (hasMethod != null) {
                            throw new MotanxFrameworkException("The interface " + interfaceClass.getName() + " has more than one method "
                                    + methodName + " , must set argumentTypes attribute.", MotanxErrorMsgConstant.FRAMEWORK_INIT_ERROR);
                        }
                        hasMethod = method;
                    }
                }
                if (hasMethod == null) {
                    throw new MotanxFrameworkException("The interface " + interfaceClass.getName() + " not found method " + methodName,
    ...
    protected String getLocalHostAddress(List<URL> registryURLs) {
        ...
        throw new MotanxServiceException("Please config local server hostname with intranet IP first!",
                MotanxErrorMsgConstant.FRAMEWORK_INIT_ERROR);
    }
