...
public class ConfigUtil {
    ...
    public static Map<String, Integer> parseExport(String export) {
        ...
        String[] protocolAndPorts = MotanxConstants.COMMA_SPLIT_PATTERN.split(export);
        for (String pp : protocolAndPorts) {
            ...
            if (ppDetail.length == 2) {
                ...
                if(MotanxConstants.PROTOCOL_INJVM.equals(ppDetail[0])){
                    pps.put(ppDetail[0], MotanxConstants.DEFAULT_INT_VALUE);
                } else{
                    int port = MathUtil.parseInt(ppDetail[0], 0);
                    if(port <= 0){
                        throw new MotanxServiceException("Export is malformed :" + export);
                    }
                }
    ...
    public static String extractProtocols(String export) {
        ...
        for (String p : protocols.keySet()) {
            sb.append(p).append(MotanxConstants.COMMA_SEPARATOR);
        }
        if (sb.length() > 0) {
